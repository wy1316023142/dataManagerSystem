...
function startHeartBeat() {
  if (client) {
    ...
    client.subscribe('/start/heartbeat', function(greeting) {
...
const clientReconnect = function() {
  ...
  socket && socket.close()
  if ((!client || !client.connected) && store.state.token) {
    ...
    // 重新添加新socket
    socket = new SockJS(`http://localhost:51515/notify`, null, {
  ...
  sockets(socketUrl).default.forEach(v => {
    ...
    socketMap[v.name] = {
      ...
      connect(headers = {}, connectCallBack = () => {}, errCallBack = () => {}) {
        if (!client) {
          socket = new SockJS(`${process.env.VUE_APP_WEBSOCKET_HOST}?access_token=${store.state.token}`, null, {
            transports: ['websocket']
          })
